...
unsafe fn b_cache_lookup_or_insert(
    ...
    if let Some(idx) = b_cache.iter().position(|e| {
        e.k == k && e.n == n && e.fp0 == fp0 && e.fp1 == fp1 && e.fp2 == fp2 && e.fp3 == fp3
    }) {
        return idx;
    }
    ...
    b_cache.push(BWeightCacheEntry {
        k,
        n,
        k_padded,
        fp0,
...
pub unsafe fn fused_dq_gemm_avx2(
    ...
    unsafe {
        ...
        let (dyn_scale, zp_a_f32) =
            dq_to_u8_rowsums_avx2(input, m, k, a_u8.as_mut_ptr(), row_sums.as_mut_ptr());
        ...
        if weight_scale_len <= 1 {
            ...
            while j + 8 <= weight_scale_len {
                ...
                _mm256_storeu_ps(
                    combined_scale.as_mut_ptr().add(j),
                    _mm256_mul_ps(dyn_scale_vec, ws),
                );
                j += 8;
            }
        ...
        if tile_n >= n {
            ...
            while n_start < n {
                ...
                let tile_scale =
                    scale_data_ptr.map(|p| if scale_len == 1 { p } else { p.add(n_start) });
                ...
                while i + 2 <= m {
                    ...
                    let out_row1 =
                        std::slice::from_raw_parts_mut(out.add((i + 1) * n + n_start), cur_n);
                    gemm_2rows_avx2(
                        &a_u8[i * k..i * k + k],
                        &a_u8[(i + 1) * k..(i + 1) * k + k],
                        tile_bt,
...
pub unsafe fn f32_to_u8_avx2(src: *const f32, dst: *mut u8, len: usize) {
    unsafe {
        ...
        while i + 16 <= len {
            ...
            _mm_storeu_si128(dst.add(i) as *mut __m128i, _mm256_castsi256_si128(u8x16));
            i += 16;
        }
        ...
        if i + 8 <= len {
            ...
            // Store lower 8 bytes
            let lo = _mm256_castsi256_si128(u8v);
            std::ptr::copy_nonoverlapping(&lo as *const __m128i as *const u8, dst.add(i), 8);
            i += 8;
        }
        while i < len {
...
pub unsafe fn transpose_b_from_f32_avx2(
    ...
    unsafe {
        ...
        // Tile dimensions for cache efficiency.
        // Tile is K-major so that B[kk, jj..jj+TILE_N] stays in L1 across N sub-blocks.
        // TILE_K=64, TILE_N=64: working set ≈ 64×64×4 = 16KB (L1 friendly).
        // Inner loop processes 8×8 sub-blocks with AVX2 block-transpose for contiguous B_T writes.
        const TILE_K: usize = 64;
        const TILE_N: usize = 64;

        // Clamp/convert constants (reused across blocks)
        let vmax255 = _mm256_set1_ps(255.0);
        let vzero_f = _mm256_setzero_ps();
        let xor_mask_epi8 = _mm_set1_epi8(0x80_u8 as i8);
        ...
        while k0 < k {
            ...
            while j0 < n {
                ...
                // Try 8×8 AVX2 sub-blocks first, then scalar for edges.
                let mut kk = k0;
                while kk < k_end {
                    let kk_end = (kk + 8).min(k_end);
                    if kk_end - kk == 8 {
                        // Full 8-row strip: vectorized over N in groups of 8.
                        let mut jj = j0;
                        while jj + 8 <= j_end {
                            // ── Load 8 rows of B[kk..kk+8, jj..jj+8] ──────────────────────────
                            // Clamp to [0,255] and convert to i32, all in-register.
                            macro_rules! load_row_i32 {
                                ($kr:expr) => {{
                                    let v = _mm256_loadu_ps(b_f32.add(($kr) * n + jj));
                                    _mm256_cvtps_epi32(_mm256_min_ps(
                                        _mm256_max_ps(v, vzero_f),
                                        vmax255,
                                    ))
                                }};
                            }
                            let r0 = load_row_i32!(kk);
                            let r1 = load_row_i32!(kk + 1);
                            let r2 = load_row_i32!(kk + 2);
                            let r3 = load_row_i32!(kk + 3);
                            let r4 = load_row_i32!(kk + 4);
                            let r5 = load_row_i32!(kk + 5);
                            let r6 = load_row_i32!(kk + 6);
                            let r7 = load_row_i32!(kk + 7);

                            // ── 8×8 i32 in-register transpose ─────────────────────────────────
                            // After transpose, column c_i = B[kk..kk+8, jj+i] (8 i32 values).
                            // These are contiguous in B_T[jj+i, kk..kk+8], so we can store
                            // them as a single 8-byte write (u64).
                            ...
                            // c0 = B[kk..kk+8, jj+0], c1 = B[kk..kk+8, jj+1], ...
                            let c0 = _mm256_permute2x128_si256(s0, s4, 0x20);
                            let c1 = _mm256_permute2x128_si256(s1, s5, 0x20);
                            let c2 = _mm256_permute2x128_si256(s2, s6, 0x20);
                            let c3 = _mm256_permute2x128_si256(s3, s7, 0x20);
                            let c4 = _mm256_permute2x128_si256(s0, s4, 0x31);
                            let c5 = _mm256_permute2x128_si256(s1, s5, 0x31);
                            let c6 = _mm256_permute2x128_si256(s2, s6, 0x31);
                            let c7 = _mm256_permute2x128_si256(s3, s7, 0x31);

                            // ── For each transposed column: pack → u8, XOR 0x80, store + sum ─
                            // Values are in [0,255] so packs/packus are non-saturating.
                            macro_rules! pack_store_sum {
                                ($ci:ident, $ji:expr) => {{
                                    let lo = _mm256_castsi256_si128($ci);
                                    let hi = _mm256_extracti128_si256($ci, 1);
                                    // i32 → i16 (saturated, safe since values ∈ [0,255])
                                    let i16v = _mm_packs_epi32(lo, hi);
                                    // i16 → u8 (unsigned saturate)
                                    let u8v = _mm_packus_epi16(i16v, _mm_setzero_si128());
                                    // XOR with 0x80 (reinterpret u8 as i8)
                                    let xored = _mm_xor_si128(u8v, xor_mask_epi8);
                                    // Store 8 contiguous bytes to B_T[jj+ji, kk..kk+8]
                                    let ptr = b_t.add((jj + $ji) * k_padded + kk);
                                    std::ptr::write_unaligned(
                                        ptr as *mut u64,
                                        _mm_extract_epi64(xored, 0) as u64,
                                    );
                                    // Vectorized col_sum: add all 8 i32 values
                                    let sum128 = _mm_add_epi32(lo, hi);
                                    let sum64 = _mm_add_epi32(
                                        sum128,
                                        _mm_shuffle_epi32(sum128, 0b01_00_11_10),
                                    );
                                    let sum32 = _mm_add_epi32(
                                        sum64,
                                        _mm_shuffle_epi32(sum64, 0b00_00_00_01),
                                    );
                                    *col_sums.add(jj + $ji) += _mm_cvtsi128_si32(sum32);
                                }};
                            }
                        ...
                        // N-column scalar remainder for this 8-row K strip
                        while jj < j_end {
                            for ki in 0..8usize {
                                let val = *b_f32.add((kk + ki) * n + jj) as u8;
                                *b_t.add(jj * k_padded + kk + ki) = val ^ 0x80;
                        ...
                        // Partial K-block (edge): scalar path
                        for ki in kk..kk_end {
                            for jj in j0..j_end {
                                let val = *b_f32.add(ki * n + jj) as u8;
                                *b_t.add(jj * k_padded + ki) = val ^ 0x80;
                                *col_sums.add(jj) += val as i32;
                            }
                        }
                        kk = kk_end;
    ...
    unsafe {
        ...
        if tile_n >= n {
            ...
            while n_start < n {
                ...
                while i + 2 <= m {
                    let out_row0 =
                        std::slice::from_raw_parts_mut(out_ptr.add(i * n + n_start), cur_n);
                    let out_row1 =
                        std::slice::from_raw_parts_mut(out_ptr.add((i + 1) * n + n_start), cur_n);
                    gemm_2rows_avx2(
                        &a_u8[i * k..i * k + k],
                        &a_u8[(i + 1) * k..(i + 1) * k + k],
                        tile_bt,
...
pub unsafe fn gemm_row_avx2(
    ...
    unsafe {
        ...
        while j + 8 <= n {
            ...
            while kk < k32 {
                // Prefetch next B cache lines to hide L2 latency across 8 streams
                let pfk = kk + 128;
                _mm_prefetch(bt0.add(pfk) as *const i8, _MM_HINT_T0);
                _mm_prefetch(bt1.add(pfk) as *const i8, _MM_HINT_T0);
                _mm_prefetch(bt2.add(pfk) as *const i8, _MM_HINT_T0);
                _mm_prefetch(bt3.add(pfk) as *const i8, _MM_HINT_T0);
                _mm_prefetch(bt4.add(pfk) as *const i8, _MM_HINT_T0);
                _mm_prefetch(bt5.add(pfk) as *const i8, _MM_HINT_T0);
                _mm_prefetch(bt6.add(pfk) as *const i8, _MM_HINT_T0);
                _mm_prefetch(bt7.add(pfk) as *const i8, _MM_HINT_T0);
            ...
            if kk < k_padded {
                ...
                iacc0 = _mm256_add_epi32(
                    iacc0,
                    _mm256_madd_epi16(
                        va,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(bt0.add(kk) as *const __m128i)),
                    ),
                );
                iacc1 = _mm256_add_epi32(
                    iacc1,
                    _mm256_madd_epi16(
                        va,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(bt1.add(kk) as *const __m128i)),
                    ),
                );
                iacc2 = _mm256_add_epi32(
                    iacc2,
                    _mm256_madd_epi16(
                        va,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(bt2.add(kk) as *const __m128i)),
                    ),
                );
                iacc3 = _mm256_add_epi32(
                    iacc3,
                    _mm256_madd_epi16(
                        va,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(bt3.add(kk) as *const __m128i)),
                    ),
                );
                iacc4 = _mm256_add_epi32(
                    iacc4,
                    _mm256_madd_epi16(
                        va,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(bt4.add(kk) as *const __m128i)),
                    ),
                );
                iacc5 = _mm256_add_epi32(
                    iacc5,
                    _mm256_madd_epi16(
                        va,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(bt5.add(kk) as *const __m128i)),
                    ),
                );
                iacc6 = _mm256_add_epi32(
                    iacc6,
                    _mm256_madd_epi16(
                        va,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(bt6.add(kk) as *const __m128i)),
                    ),
                );
                iacc7 = _mm256_add_epi32(
                    iacc7,
                    _mm256_madd_epi16(
                        va,
                        _mm256_cvtepi8_epi16(_mm_loadu_si128(bt7.add(kk) as *const __m128i)),
                    ),
                );
            }
            ...
            // Vectorized post-processing: correction, scale, bias, relu for 8 columns
            let cs0123 = _mm_loadu_si128(col_sums_ptr.add(j) as *const __m128i);
            let cs4567 = _mm_loadu_si128(col_sums_ptr.add(j + 4) as *const __m128i);
            let k_zp_b_v = _mm_set1_epi32(k_zp_b);
            let zp_a_v = _mm_set1_epi32(zp_a);
            let row_corr_v = _mm_set1_epi32(row_corr);
            let corr0123 = _mm_sub_epi32(
                row_corr_v,
                _mm_mullo_epi32(zp_a_v, _mm_sub_epi32(cs0123, k_zp_b_v)),
            );
            let corr4567 = _mm_sub_epi32(
                row_corr_v,
                _mm_mullo_epi32(zp_a_v, _mm_sub_epi32(cs4567, k_zp_b_v)),
            );
            let combined = _mm256_set_m128i(
                _mm_add_epi32(dots_4567, corr4567),
                _mm_add_epi32(dots_0123, corr0123),
            );
            let mut vf = _mm256_cvtepi32_ps(combined);
            if let Some(sp) = scale_data_ptr {
                if scale_len == 1 {
                    vf = _mm256_mul_ps(vf, _mm256_set1_ps(*sp));
                } else {
                    vf = _mm256_mul_ps(vf, _mm256_loadu_ps(sp.add(j)));
                }
            }
            if let Some(bp) = bias_data_ptr {
                vf = _mm256_add_ps(vf, _mm256_loadu_ps(bp.add(j)));
            }
            if apply_relu {
                vf = _mm256_max_ps(vf, _mm256_setzero_ps());
            }
            _mm256_storeu_ps(out_row.as_mut_ptr().add(j), vf);
...
/// Uses VPMADDUBSW → VPMADDWD(ones) cascade for maximum throughput.
/// VPMADDUBSW processes 32 u8×i8 pairs per instruction (vs 16 with cvtepu8+pmaddwd).
/// Minor i16 saturation possible but acceptable for int8 inference.
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2", enable = "fma")]
pub unsafe fn gemm_2rows_avx2(
    ...
    unsafe {
        ...
        // Pre-widen A from u8 to i16 — removes cvtepu8_epi16 from the hot K loop.
        let mut a0_i16 = [0i16; 2080];
        let mut a1_i16 = [0i16; 2080];
        {
            let mut wk = 0;
            while wk + 16 <= k_padded {
                let v0 = _mm256_cvtepu8_epi16(_mm_loadu_si128(a0_ptr.add(wk) as *const __m128i));
                let v1 = _mm256_cvtepu8_epi16(_mm_loadu_si128(a1_ptr.add(wk) as *const __m128i));
                _mm256_storeu_si256(a0_i16.as_mut_ptr().add(wk) as *mut __m256i, v0);
                _mm256_storeu_si256(a1_i16.as_mut_ptr().add(wk) as *mut __m256i, v1);
                wk += 16;
            }
        }
        let a0_i16_ptr = a0_i16.as_ptr();
        let a1_i16_ptr = a1_i16.as_ptr();
        ...
        // 8-column unrolling with 2 rows = 16 accumulators (fills all YMM registers).
        // Cuts N-loop iterations in half vs 4-col, exposes more ILP, and uses full-width
        // 256-bit epilog (correction/scale/bias/relu) for 8 outputs at once.
        while j + 8 <= n {
            let bt0 = b_t_ptr.add(j * k_padded);
            let bt1 = b_t_ptr.add((j + 1) * k_padded);
            let bt2 = b_t_ptr.add((j + 2) * k_padded);
            ...
            let mut r0c4 = _mm256_setzero_si256();
            let mut r0c5 = _mm256_setzero_si256();
            let mut r0c6 = _mm256_setzero_si256();
            let mut r0c7 = _mm256_setzero_si256();
            ...
            let mut r1c4 = _mm256_setzero_si256();
            let mut r1c5 = _mm256_setzero_si256();
            let mut r1c6 = _mm256_setzero_si256();
            let mut r1c7 = _mm256_setzero_si256();

            let mut kk = 0;
            let k32 = k_padded & !31;

            // Main loop: 32 K elements per iteration, 8 columns × 2 rows = 16 accumulators.
            // A is pre-widened to i16. B loaded once per column, shared across both rows.
            while kk < k32 {
                // Software prefetch: request next B cache lines (4 iterations = 128 bytes ahead).
                // This hides L2 latency (~12 cycles) for streams the hardware prefetcher may miss.
                let pfk = kk + 128;
                _mm_prefetch(bt0.add(pfk) as *const i8, _MM_HINT_T0);
                _mm_prefetch(bt1.add(pfk) as *const i8, _MM_HINT_T0);
                _mm_prefetch(bt2.add(pfk) as *const i8, _MM_HINT_T0);
                ...
                let a0_lo = _mm256_loadu_si256(a0_i16_ptr.add(kk) as *const __m256i);
                let a0_hi = _mm256_loadu_si256(a0_i16_ptr.add(kk + 16) as *const __m256i);
                let a1_lo = _mm256_loadu_si256(a1_i16_ptr.add(kk) as *const __m256i);
                let a1_hi = _mm256_loadu_si256(a1_i16_ptr.add(kk + 16) as *const __m256i);

                macro_rules! acc8col {
                    ($r0:ident, $r1:ident, $bt:ident) => {{
                        let bf = _mm256_loadu_si256($bt.add(kk) as *const __m256i);
                        let vb_lo = _mm256_cvtepi8_epi16(_mm256_castsi256_si128(bf));
                        let vb_hi = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(bf, 1));
                        $r0 = _mm256_add_epi32($r0, _mm256_madd_epi16(a0_lo, vb_lo));
                        $r0 = _mm256_add_epi32($r0, _mm256_madd_epi16(a0_hi, vb_hi));
                        $r1 = _mm256_add_epi32($r1, _mm256_madd_epi16(a1_lo, vb_lo));
                        $r1 = _mm256_add_epi32($r1, _mm256_madd_epi16(a1_hi, vb_hi));
                    }};
                }

                acc8col!(r0c0, r1c0, bt0);
                acc8col!(r0c1, r1c1, bt1);
                acc8col!(r0c2, r1c2, bt2);
                acc8col!(r0c3, r1c3, bt3);
                acc8col!(r0c4, r1c4, bt4);
                acc8col!(r0c5, r1c5, bt5);
                acc8col!(r0c6, r1c6, bt6);
                acc8col!(r0c7, r1c7, bt7);
            ...
            // 16-byte remainder
            if kk < k_padded {
                let va0 = _mm256_loadu_si256(a0_i16_ptr.add(kk) as *const __m256i);
                let va1 = _mm256_loadu_si256(a1_i16_ptr.add(kk) as *const __m256i);

                macro_rules! acc8col_rem {
                    ($r0:ident, $r1:ident, $bt:ident) => {{
                        let vb =
                            _mm256_cvtepi8_epi16(_mm_loadu_si128($bt.add(kk) as *const __m128i));
                        $r0 = _mm256_add_epi32($r0, _mm256_madd_epi16(va0, vb));
                        $r1 = _mm256_add_epi32($r1, _mm256_madd_epi16(va1, vb));
                    }};
                }

                acc8col_rem!(r0c0, r1c0, bt0);
                acc8col_rem!(r0c1, r1c1, bt1);
                acc8col_rem!(r0c2, r1c2, bt2);
                acc8col_rem!(r0c3, r1c3, bt3);
                acc8col_rem!(r0c4, r1c4, bt4);
                acc8col_rem!(r0c5, r1c5, bt5);
                acc8col_rem!(r0c6, r1c6, bt6);
                acc8col_rem!(r0c7, r1c7, bt7);
            }

            // Horizontal reduce: two groups of 4 → two 128-bit results per row,
            // then combine to a single 256-bit vector for full-width epilog.
            macro_rules! reduce4to128 {
                ($c0:ident, $c1:ident, $c2:ident, $c3:ident) => {{
                    let h01 = _mm256_hadd_epi32($c0, $c1);
                    let h23 = _mm256_hadd_epi32($c2, $c3);
                    let h0123 = _mm256_hadd_epi32(h01, h23);
                    _mm_add_epi32(
                        _mm256_castsi256_si128(h0123),
                        _mm256_extracti128_si256(h0123, 1),
                    )
                }};
            }

            let dots_0123_r0 = reduce4to128!(r0c0, r0c1, r0c2, r0c3);
            let dots_4567_r0 = reduce4to128!(r0c4, r0c5, r0c6, r0c7);
            let dots_0123_r1 = reduce4to128!(r1c0, r1c1, r1c2, r1c3);
            let dots_4567_r1 = reduce4to128!(r1c4, r1c5, r1c6, r1c7);

            // Combine to 256-bit: process all 8 columns in a single pass.
            let dots_r0 = _mm256_set_m128i(dots_4567_r0, dots_0123_r0);
            let dots_r1 = _mm256_set_m128i(dots_4567_r1, dots_0123_r1);

            // Vectorized 8-column correction, scale, bias, relu using 256-bit ops.
            let cs = _mm256_loadu_si256(col_sums_ptr.add(j) as *const __m256i);
            let zp_corr = _mm256_mullo_epi32(
                _mm256_set1_epi32(zp_a),
                _mm256_sub_epi32(cs, _mm256_set1_epi32(k_zp_b)),
            );
            let corr_r0 = _mm256_sub_epi32(
                _mm256_add_epi32(dots_r0, _mm256_set1_epi32(row_corr0)),
                zp_corr,
            );
            let corr_r1 = _mm256_sub_epi32(
                _mm256_add_epi32(dots_r1, _mm256_set1_epi32(row_corr1)),
                zp_corr,
            );

            let mut f_r0 = _mm256_cvtepi32_ps(corr_r0);
            let mut f_r1 = _mm256_cvtepi32_ps(corr_r1);

            if let Some(sp) = scale_data_ptr {
                let sv = if scale_len == 1 {
                    _mm256_set1_ps(*sp)
                } else {
                    _mm256_loadu_ps(sp.add(j))
                };
                f_r0 = _mm256_mul_ps(f_r0, sv);
                f_r1 = _mm256_mul_ps(f_r1, sv);
            }
            if let Some(bp) = bias_data_ptr {
                let bv = _mm256_loadu_ps(bp.add(j));
                f_r0 = _mm256_add_ps(f_r0, bv);
                f_r1 = _mm256_add_ps(f_r1, bv);
            }
            if apply_relu {
                let zero = _mm256_setzero_ps();
                f_r0 = _mm256_max_ps(f_r0, zero);
                f_r1 = _mm256_max_ps(f_r1, zero);
            }

            _mm256_storeu_ps(out_row0.as_mut_ptr().add(j), f_r0);
            _mm256_storeu_ps(out_row1.as_mut_ptr().add(j), f_r1);

            j += 8;
        }

        // 4-column unrolling with 2 rows = 8 accumulators
        while j + 4 <= n {
            let bt0 = b_t_ptr.add(j * k_padded);
            let bt1 = b_t_ptr.add((j + 1) * k_padded);
            let bt2 = b_t_ptr.add((j + 2) * k_padded);
            ...
            // Main loop: 32 K elements per iteration
            // A is pre-widened to i16 (no cvtepu8 in loop). B is widened per column.
            while kk < k32 {
                let a0_lo = _mm256_loadu_si256(a0_i16_ptr.add(kk) as *const __m256i);
                let a0_hi = _mm256_loadu_si256(a0_i16_ptr.add(kk + 16) as *const __m256i);
                let a1_lo = _mm256_loadu_si256(a1_i16_ptr.add(kk) as *const __m256i);
            ...
            // Vectorized horizontal reduce + epilog
            let h01_r0 = _mm256_hadd_epi32(r0c0, r0c1);
            let h23_r0 = _mm256_hadd_epi32(r0c2, r0c3);
            let h0123_r0 = _mm256_hadd_epi32(h01_r0, h23_r0);
            let dots_r0 = _mm_add_epi32(
                _mm256_castsi256_si128(h0123_r0),
                _mm256_extracti128_si256(h0123_r0, 1),
            );

            let h01_r1 = _mm256_hadd_epi32(r1c0, r1c1);
            let h23_r1 = _mm256_hadd_epi32(r1c2, r1c3);
            let h0123_r1 = _mm256_hadd_epi32(h01_r1, h23_r1);
            let dots_r1 = _mm_add_epi32(
                _mm256_castsi256_si128(h0123_r1),
                _mm256_extracti128_si256(h0123_r1, 1),
            );

            // Vectorized correction
            let cs = _mm_loadu_si128(col_sums_ptr.add(j) as *const __m128i);
            let cs_adj = _mm_sub_epi32(cs, _mm_set1_epi32(k_zp_b));
            let zp_corr = _mm_mullo_epi32(_mm_set1_epi32(zp_a), cs_adj);

            let corr_r0 = _mm_sub_epi32(_mm_add_epi32(dots_r0, _mm_set1_epi32(row_corr0)), zp_corr);
            let corr_r1 = _mm_sub_epi32(_mm_add_epi32(dots_r1, _mm_set1_epi32(row_corr1)), zp_corr);

            let mut f_r0 = _mm_cvtepi32_ps(corr_r0);
            let mut f_r1 = _mm_cvtepi32_ps(corr_r1);

            if let Some(sp) = scale_data_ptr {
                let sv = if scale_len == 1 {
                    _mm_set1_ps(*sp)
                } else {
                    _mm_loadu_ps(sp.add(j))
                };
                f_r0 = _mm_mul_ps(f_r0, sv);
                f_r1 = _mm_mul_ps(f_r1, sv);
            }
            if let Some(bp) = bias_data_ptr {
                let bv = _mm_loadu_ps(bp.add(j));
                f_r0 = _mm_add_ps(f_r0, bv);
                f_r1 = _mm_add_ps(f_r1, bv);
            }
            if apply_relu {
                let zero = _mm_setzero_ps();
                f_r0 = _mm_max_ps(f_r0, zero);
                f_r1 = _mm_max_ps(f_r1, zero);
            }

            _mm_storeu_ps(out_row0.as_mut_ptr().add(j), f_r0);
            _mm_storeu_ps(out_row1.as_mut_ptr().add(j), f_r1);
...
/// VPMADDUBSW-based 2-row GEMM kernel: processes 32 K elements per iteration (2x throughput).
/// Combines 2-row B sharing with VPMADDUBSW (32 bytes/iter) for maximum throughput.
/// Uses 8 accumulators (2 rows × 4 columns) to keep all 16 YMM registers busy.
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2", enable = "fma")]
pub unsafe fn gemm_2rows_avx2_v2(
    a_row0: &[u8],
    a_row1: &[u8],
    b_t_ptr: *const u8,
    k: usize,
    ...
    unsafe {
        let a0_ptr: *const u8;
        let a1_ptr: *const u8;
        let mut a0_padded = [0u8; 4096];
        let mut a1_padded = [0u8; 4096];
        if k_aligned {
            a0_ptr = a_row0.as_ptr();
            a1_ptr = a_row1.as_ptr();
        } else {
        ...
        let mask_7f = _mm256_set1_epi8(0x7F_u8 as i8);
        let k32 = k_padded & !31;
        ...
        while j + 4 <= n {
            ...
            let mut kk = 0;
            // Main loop: saturation-free VPMADDUBSW via A split
            // A = (A & 0x7F) + (A & 0x80), each half safe with MADDUBS
            while kk < k32 {
                let va0 = _mm256_loadu_si256(a0_ptr.add(kk) as *const __m256i);
                let va1 = _mm256_loadu_si256(a1_ptr.add(kk) as *const __m256i);
                let a0_lo = _mm256_and_si256(va0, mask_7f);
                let a0_hi = _mm256_andnot_si256(mask_7f, va0);
                let a1_lo = _mm256_and_si256(va1, mask_7f);
                let a1_hi = _mm256_andnot_si256(mask_7f, va1);

                let vb0 = _mm256_loadu_si256(bt0.add(kk) as *const __m256i);
                let vb1 = _mm256_loadu_si256(bt1.add(kk) as *const __m256i);
                let vb2 = _mm256_loadu_si256(bt2.add(kk) as *const __m256i);
                let vb3 = _mm256_loadu_si256(bt3.add(kk) as *const __m256i);

                // Row 0: split MADDUBS for each column
                r0c0 = _mm256_add_epi32(
                    r0c0,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a0_lo, vb0), ones_16),
                );
                r0c0 = _mm256_add_epi32(
                    r0c0,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a0_hi, vb0), ones_16),
                );
                r0c1 = _mm256_add_epi32(
                    r0c1,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a0_lo, vb1), ones_16),
                );
                r0c1 = _mm256_add_epi32(
                    r0c1,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a0_hi, vb1), ones_16),
                );
                r0c2 = _mm256_add_epi32(
                    r0c2,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a0_lo, vb2), ones_16),
                );
                r0c2 = _mm256_add_epi32(
                    r0c2,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a0_hi, vb2), ones_16),
                );
                r0c3 = _mm256_add_epi32(
                    r0c3,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a0_lo, vb3), ones_16),
                );
                r0c3 = _mm256_add_epi32(
                    r0c3,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a0_hi, vb3), ones_16),
                );

                // Row 1: split MADDUBS for each column (B loads shared)
                r1c0 = _mm256_add_epi32(
                    r1c0,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a1_lo, vb0), ones_16),
                );
                r1c0 = _mm256_add_epi32(
                    r1c0,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a1_hi, vb0), ones_16),
                );
                r1c1 = _mm256_add_epi32(
                    r1c1,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a1_lo, vb1), ones_16),
                );
                r1c1 = _mm256_add_epi32(
                    r1c1,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a1_hi, vb1), ones_16),
                );
                r1c2 = _mm256_add_epi32(
                    r1c2,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a1_lo, vb2), ones_16),
                );
                r1c2 = _mm256_add_epi32(
                    r1c2,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a1_hi, vb2), ones_16),
                );
                r1c3 = _mm256_add_epi32(
                    r1c3,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a1_lo, vb3), ones_16),
                );
                r1c3 = _mm256_add_epi32(
                    r1c3,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a1_hi, vb3), ones_16),
                );

                kk += 32;
            }

            // 16-byte remainder using cvtepu8/cvtepi8 (saturation-free)
            if kk < k_padded {
        ...
        while j < n {
            ...
            let mut kk = 0;
            while kk < k32 {
                let vb = _mm256_loadu_si256(bt.add(kk) as *const __m256i);
                let va0 = _mm256_loadu_si256(a0_ptr.add(kk) as *const __m256i);
                let va1 = _mm256_loadu_si256(a1_ptr.add(kk) as *const __m256i);
                let a0_lo = _mm256_and_si256(va0, mask_7f);
                let a0_hi = _mm256_andnot_si256(mask_7f, va0);
                ...
                iacc0 = _mm256_add_epi32(
                    iacc0,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a0_lo, vb), ones_16),
                );
                iacc0 = _mm256_add_epi32(
                    iacc0,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a0_hi, vb), ones_16),
                );
                iacc1 = _mm256_add_epi32(
                    iacc1,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a1_lo, vb), ones_16),
                );
                iacc1 = _mm256_add_epi32(
                    iacc1,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a1_hi, vb), ones_16),
                );
                kk += 32;
            }
            if kk < k_padded {
...
pub unsafe fn gemm_row_avx2_v2(
    ...
    unsafe {
        ...
        let mut j = 0;
        // 8-column unrolling with saturation-free VPMADDUBSW
        while j + 8 <= n {
            let bt0 = b_t_ptr.add(j * k_padded);
            let bt1 = b_t_ptr.add((j + 1) * k_padded);
            let bt2 = b_t_ptr.add((j + 2) * k_padded);
            ...
            // Main loop: 32 elements per iteration, saturation-free split
            let mut kk = 0;
            while kk < k32 {
                let va = _mm256_loadu_si256(a_ptr.add(kk) as *const __m256i);
                let a_lo = _mm256_and_si256(va, mask_7f);
                let a_hi = _mm256_andnot_si256(mask_7f, va);

                let vb0 = _mm256_loadu_si256(bt0.add(kk) as *const __m256i);
                iacc0 = _mm256_add_epi32(
                    iacc0,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_lo, vb0), ones_16),
                );
                iacc0 = _mm256_add_epi32(
                    iacc0,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_hi, vb0), ones_16),
                );
                let vb1 = _mm256_loadu_si256(bt1.add(kk) as *const __m256i);
                iacc1 = _mm256_add_epi32(
                    iacc1,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_lo, vb1), ones_16),
                );
                iacc1 = _mm256_add_epi32(
                    iacc1,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_hi, vb1), ones_16),
                );
                let vb2 = _mm256_loadu_si256(bt2.add(kk) as *const __m256i);
                iacc2 = _mm256_add_epi32(
                    iacc2,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_lo, vb2), ones_16),
                );
                iacc2 = _mm256_add_epi32(
                    iacc2,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_hi, vb2), ones_16),
                );
                let vb3 = _mm256_loadu_si256(bt3.add(kk) as *const __m256i);
                iacc3 = _mm256_add_epi32(
                    iacc3,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_lo, vb3), ones_16),
                );
                iacc3 = _mm256_add_epi32(
                    iacc3,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_hi, vb3), ones_16),
                );
                let vb4 = _mm256_loadu_si256(bt4.add(kk) as *const __m256i);
                iacc4 = _mm256_add_epi32(
                    iacc4,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_lo, vb4), ones_16),
                );
                iacc4 = _mm256_add_epi32(
                    iacc4,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_hi, vb4), ones_16),
                );
                let vb5 = _mm256_loadu_si256(bt5.add(kk) as *const __m256i);
                iacc5 = _mm256_add_epi32(
                    iacc5,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_lo, vb5), ones_16),
                );
                iacc5 = _mm256_add_epi32(
                    iacc5,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_hi, vb5), ones_16),
                );
                let vb6 = _mm256_loadu_si256(bt6.add(kk) as *const __m256i);
                iacc6 = _mm256_add_epi32(
                    iacc6,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_lo, vb6), ones_16),
                );
                iacc6 = _mm256_add_epi32(
                    iacc6,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_hi, vb6), ones_16),
                );
                let vb7 = _mm256_loadu_si256(bt7.add(kk) as *const __m256i);
                iacc7 = _mm256_add_epi32(
                    iacc7,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_lo, vb7), ones_16),
                );
                iacc7 = _mm256_add_epi32(
                    iacc7,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_hi, vb7), ones_16),
                );
        ...
        while j < n {
            ...
            while kk < k32 {
                ...
                iacc = _mm256_add_epi32(
                    iacc,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_lo, vb), ones_16),
                );
                iacc = _mm256_add_epi32(
                    iacc,
                    _mm256_madd_epi16(_mm256_maddubs_epi16(a_hi, vb), ones_16),
                );
                kk += 32;
            }
            if kk < k_padded {
