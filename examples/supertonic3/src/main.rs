...
impl<'a> SupertonicTts<'a> {
    ...
    pub fn synthesize(
        ...
        eprintln!(
            "[OVERHEAD] load_style: {:.2}ms",
            t_overhead.elapsed().as_secs_f64() * 1000.0
        );
        ...
        for chunk in chunks {
            ...
            eprintln!(
                "[OVERHEAD] tokenization: {:.2}ms",
                t_tok.elapsed().as_secs_f64() * 1000.0
            );
            let bsz = 1;
            ...
            eprintln!(
                "[STAGE] DurationPredictor: {:.2}ms",
                t_dp.elapsed().as_secs_f64() * 1000.0
            );
            ...
            eprintln!(
                "[STAGE] TextEncoder: {:.2}ms",
                t_te.elapsed().as_secs_f64() * 1000.0
            );
            ...
            eprintln!(
                "[STAGE] VectorEstimator total: {:.2}ms",
                t_ve.elapsed().as_secs_f64() * 1000.0
            );
...
fn main() -> Result<()> {
    ...
    let weights_dir = Path::new("examples/supertonic3/onnx");
    let gen_dir = Path::new("examples/supertonic3/src");
    let voice_styles_dir = Path::new("examples/supertonic3/voice_styles");
    ...
    let audio = tts.synthesize(&text, lang, style_name, 1.05, 5)?;
